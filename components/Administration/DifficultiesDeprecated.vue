...
export default {
  ...
  methods: {
    ...
    axiosGet() {
      const promise = this.$database.axios.get("http://localhost:8081/admin/difficulty");
      const dataPromise = promise.then((resp) => resp.data);
      return dataPromise;
    },

    axiosPost(obj) {
      const promise = this.$database.axios.post("http://localhost:8081/admin/difficulty", obj);
      return promise;
    },

    axiosDelete(index) {
      const promise = this.$database.axios.delete("http://localhost:8081/admin/difficulty", {
        params: {
          id: index,
        },
      });
    ...
    axiosPut(obj) {
      const promise = this.$database.axios.put("http://localhost:8081/admin/difficulty", obj);
      return promise;
    },
  },
  mounted() {
